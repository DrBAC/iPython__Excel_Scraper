...
print('Excel File Name',',','CL',',','CW',',','Mobility',',','SDev',',','On/Off',',','VTO',',','VTH',',','Yield',',','Capacitance')
...
with open(date, "w", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["File", "Date of test", "CL", "CW", "Mobility", "SDev", "On/Off", "VTO", "VTH", "Yield", "Capacitance", "Comments"])
            ...
            if fname.endswith('.xlsm') or fname.endswith('.xlsx'):
                ...
                if 'validated Summary Data' in names:
                    ...
                    # this lot that follow get looped
                        ...
                        channel_data = (fname,test_date,ChannelL,ChannelW,MobilityMed, MobSDev,Ionoff,VTO,VTH,Yield,Capacitance,Comments)
                        substrate_data.append(channel_data)
                        writer.writerow(channel_data)                       
                        print(fname,',',test_date,',',ChannelL,',',ChannelW,',',MobilityMed,',',MobSDev,',',Ionoff,',',VTO,',',VTH,',',Yield,',',Capacitance)                  
                        continue
                    print('-'*100,substrate)  ####### DON'T EDIT THIS VERSION !!!!!!!!!!                    
        continue 
