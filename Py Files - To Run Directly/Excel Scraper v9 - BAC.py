        ...
        for fname in files:
            if fname.endswith('.xlsm') or fname.endswith('.xlsx'):
                ...
                if 'validated Summary Data' in names:
                    ...
                    Comment_Pattern = re.compile('comment', re.I)
                    Comment_ORIGIN = SearchXLWithRE(Comment_Pattern)
                    ...
                    Cap_Pattern = re.compile('Cap', re.I)
                    Cap_ORIGIN = SearchXLWithRE(Cap_Pattern)
                    ...
                    for r in range(1,5):
                        ...
                        Comments = ws.cell(row=Comment_ORIGIN[0],column=Comment_ORIGIN[1]+1).value
                        Capacitance = ws.cell(row=Cap_ORIGIN[0]+1,column=Cap_ORIGIN[1]).value
                        # no looping 1-4
                        ...
                        VTO = DataSeeker(VTO_ORIGIN,0,0,"{:.2f}")    
                        ...
                        channel_data = (fname,test_date,ChannelL,ChannelW,MobilityMed,
                                        MobSDev,Ionoff,VTO,VTH,Yield,Capacitance,Comments)
                        
                        substrate_data.append(channel_data)
                        
                        writer.writerow(channel_data)
                       
                        print(fname,',',test_date,',',ChannelL,',',ChannelW,',',MobilityMed,',',MobSDev,
                              ',',Ionoff,',',VTO,',',VTH,',',Yield,',',Capacitance)
